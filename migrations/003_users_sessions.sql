...
--
-- Tables: users_fleet, sessions, placement_decisions.
...
CREATE TABLE IF NOT EXISTS users_fleet (
  ...
  fixed_ip      INET,
  pinned_chr_id BIGINT      REFERENCES chr_nodes(id),
  created_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT uq_users_fleet_username UNIQUE (username)
);
CREATE INDEX IF NOT EXISTS idx_users_fleet_customer ON users_fleet (customer_id);
CREATE INDEX IF NOT EXISTS idx_users_fleet_realm    ON users_fleet (realm);
-- Hot index for the rebalance planner — most users are immovable.
CREATE INDEX IF NOT EXISTS idx_users_movable
  ON users_fleet (movable) WHERE movable;
...
CREATE INDEX IF NOT EXISTS idx_sessions_chr
  ON sessions (chr_id) WHERE state = 'active';
...
CREATE INDEX IF NOT EXISTS idx_pd_user
  ON placement_decisions (username, decided_at DESC);
CREATE INDEX IF NOT EXISTS idx_pd_kind_decided
  ON placement_decisions (kind, decided_at DESC);
