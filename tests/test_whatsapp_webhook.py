...
def test_post_status_delivered_updates_row_and_stores_event(client, app):
    ...
    res = client.post("/api/whatsapp/webhook", json=_status_payload("delivered"))
    assert res.status_code == 200
    body = res.get_json()
...
def test_post_status_read_updates_row(client, app):
    ...
    res = client.post("/api/whatsapp/webhook", json=_status_payload("read"))
    assert res.status_code == 200
...
def test_post_status_failed_stores_error(client, app):
    ...
    res = client.post("/api/whatsapp/webhook", json=_status_payload("failed", errors=errors))
    assert res.status_code == 200
    assert res.get_json()["processed"] == 1
...
def test_post_distinct_statuses_for_same_message_each_stored(client, app):
    ...
    for status in ("sent", "delivered", "read"):
        res = client.post("/api/whatsapp/webhook", json=_status_payload(status))
        assert res.status_code == 200
...
def test_post_empty_json_object_returns_200(client, app):
    ...
    res = client.post("/api/whatsapp/webhook", json={})
    assert res.status_code == 200
...
def test_post_with_valid_signature_processes(client, app):
    """Account WITH a stored app secret + a correct signature -> processed."""
    import hashlib
    ...
    signature = "sha256=" + _hmac.new(APP_SECRET.encode("utf-8"), raw, hashlib.sha256).hexdigest()
...
def test_post_with_bad_signature_stores_mismatch_not_processed(client, app):
    ...
    with app.app_context():
        event = WhatsAppWebhookEvent.query.filter_by(provider_message_id=WAMID).one()
        assert event.processing_error == "signature_mismatch"
        assert event.processed is False
        # The queue row was NOT advanced.
        row = db.session.get(WhatsAppMessageQueue, row_id)
