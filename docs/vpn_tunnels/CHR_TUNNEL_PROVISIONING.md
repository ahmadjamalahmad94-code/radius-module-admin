...
ومحصورة بالمسؤول العام ومُدقَّقة. القراءة أولاً ولا تنهار الصفحة عند تعذّر الوصول —
تُظهر شارة حالة. الطبقة في `app/services/chr_console.py` (لا ترفع، تعيد `{ok, …}`).

## 8) قرارات/نقاط مؤجَّلة (لم تُخترَع)
