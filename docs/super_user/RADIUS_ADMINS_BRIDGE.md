...
```jsonc
{
  ...
  "full_snapshot": true,          // v2 (اختياري، الافتراضي true): true = جرد كامل → تقليم بالغياب
  "admins": [
    {
    ...
    { "id": 7, "deleted": true }        // v2 (اختياري): شاهدة حذف صريحة → تُزيل الصفّ فوراً
  ]
}
```
...
> **تحديث v2 — البلاغ جردٌ كامل + شواهد حذف (إصلاح «المحذوف لسا مسجل»):**
> صارت اللوحة **تقلّم** لقطات الأدمن المحذوفين على الراديوس بطريقتين متكاملتين:
> 1. **جرد كامل (الافتراضي):** أرسِل كل أدمنيات الراديوس في كل استدعاء؛ أيُّ لقطة
>    غائبة عن البلاغ تُحذف. هذا كافٍ لوحده — العملاء القدامى الذين يرسلون جرداً
>    كاملاً يحصلون على حذفٍ صحيح دون أي تغيير.
> 2. **شاهدة حذف صريحة (tombstone):** عنصرٌ يحمل `"deleted": true` (أو
>    `"op": "delete"`) بمعرّفٍ صالح يُحذف صفُّه فوراً — يعمل حتى في دفعةٍ
>    **جزئية** (`"full_snapshot": false`)، فيمكن الإبلاغ عن حذفٍ مفرد بلا إرسال
>    الجرد كلّه.
>
> إن أرسل الراديوس دفعةً جزئية فليضبط `"full_snapshot": false` كي توقف اللوحةُ
> التقليمَ بالغياب لتلك الدفعة (تبقى الشواهد الصريحة عاملة). حارس أمان على اللوحة:
> لا تُقلّم بالغياب أبداً على بلاغٍ خالٍ/بلا معرّفات صالحة (كي لا يمحو بلاغٌ معطوب
> الجردَ كلَّه).
