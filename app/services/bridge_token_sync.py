...
def _bootstrap_seed_for(license: License) -> str:
    """Initial plaintext for an unseeded license — a fresh random token.

    NOTE: main retired the legacy HMAC-signed bridge (verify_license_signature
    is bearer-only now), so there is no derived ``license_integration_secret``
    to reuse as a seed. The bridge token is a synced VALUE only; it is not the
    auth secret, so a random bootstrap is correct and keeps signing untouched.
    """
    del license  # seed is independent of the license in bearer-auth mode
    return _generate_plaintext()
