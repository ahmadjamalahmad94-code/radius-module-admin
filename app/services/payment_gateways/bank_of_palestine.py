"""Bank of Palestine (بنك فلسطين) adapter — STUB pending API credentials.

BoP's online-payments API is a classic 3-D-secure hosted form: we open a
session, redirect the customer to the bank's checkout, and receive a signed
callback. The single outbound HTTP call site is marked
``# TODO(payment-gateways)``.
"""
from __future__ import annotations

import hmac
import hashlib
from decimal import Decimal
from typing import Any

from .base import CreatePaymentInput, GatewayResult, NotConfiguredError
from . import _http


class BankOfPalestineAdapter:
    name = "bank_of_palestine"
    label_ar = "بنك فلسطين"
    cred_keys = ("api_url", "terminal_id", "username", "password", "shared_key")

    def configured(self, creds: dict[str, str]) -> bool:
        return all((creds.get(k) or "").strip() for k in self.cred_keys)

    def create_payment(self, creds: dict[str, str], data: CreatePaymentInput) -> GatewayResult:
        if not self.configured(creds):
            raise NotConfiguredError(f"{self.name} not configured")
        amount, code = self._validate_amount(data.amount, data.currency)
        if code:
            return GatewayResult(False, code=code, message=self._msg(code))

        url = creds["api_url"].rstrip("/") + "/payment/session"
        payload: dict[str, Any] = {
            "terminal_id": creds["terminal_id"],
            "order_id": data.reference,
            "amount": str(amount),
            "currency": data.currency,
            "description": data.description,
            "return_url": data.callback_url,
        }
        headers = {
            "Content-Type": "application/json",
            "X-Auth-User": creds["username"],
            "X-Auth-Pass": creds["password"],
            "X-Signature": self._sign(creds["shared_key"], payload),
        }

        # TODO(payment-gateways): Bank of Palestine create-session call. Single
        # network seam. Owner replaces `api_url` (sandbox vs production).
        # Expected response shape:
        #   { "session_id": "<id>", "redirect_url": "https://...",
        #     "state": "PENDING|APPROVED|DECLINED" }
        result = _http.post_json(url, payload=payload, headers=headers, timeout=20.0)
        if result.error:
            return GatewayResult(False, code="http_error", message=self._msg("http_error"),
                                 meta={"error": result.error})
        if not result.ok:
            return GatewayResult(False, code="provider_error", message=self._msg("provider_error"),
                                 meta={"status": result.status})
        body = result.body or {}
        return GatewayResult(
            True, code="ok", message="تم فتح جلسة الدفع لدى بنك فلسطين.",
            provider_payment_id=str(body.get("session_id", "")),
            redirect_url=str(body.get("redirect_url", "")),
            status=self._normalize_status(str(body.get("state", "PENDING")).upper()),
            meta={"raw": body},
        )

    def verify_callback(self, creds: dict[str, str], raw: dict[str, Any]) -> GatewayResult:
        if not self.configured(creds):
            raise NotConfiguredError(f"{self.name} not configured")
        provider_id = str(raw.get("session_id") or "").strip()
        if not provider_id:
            return GatewayResult(False, code="missing_payment_id", message=self._msg("missing_payment_id"))
        if not self._signature_ok(creds, raw):
            return GatewayResult(False, code="invalid_signature", message=self._msg("invalid_signature"))
        return GatewayResult(
            True, code="ok", message="تم التحقق من رد بنك فلسطين.",
            provider_payment_id=provider_id,
            status=self._normalize_status(str(raw.get("state") or raw.get("status") or "").upper()),
            meta={"raw_state": raw.get("state") or raw.get("status")},
        )

    def status(self, creds: dict[str, str], provider_payment_id: str) -> GatewayResult:
        if not self.configured(creds):
            raise NotConfiguredError(f"{self.name} not configured")
        if not (provider_payment_id or "").strip():
            return GatewayResult(False, code="missing_payment_id", message=self._msg("missing_payment_id"))
        url = creds["api_url"].rstrip("/") + f"/payment/session/{provider_payment_id}"
        headers = {
            "X-Auth-User": creds["username"],
            "X-Auth-Pass": creds["password"],
        }

        # TODO(payment-gateways): Bank of Palestine status-poll call.
        result = _http.get_json(url, headers=headers, timeout=20.0)
        if result.error:
            return GatewayResult(False, code="http_error", message=self._msg("http_error"),
                                 meta={"error": result.error})
        if not result.ok:
            return GatewayResult(False, code="provider_error", message=self._msg("provider_error"),
                                 meta={"status": result.status})
        body = result.body or {}
        return GatewayResult(
            True, code="ok", message="تم استرجاع الحالة من بنك فلسطين.",
            provider_payment_id=str(body.get("session_id", provider_payment_id)),
            status=self._normalize_status(str(body.get("state", "PENDING")).upper()),
            meta={"raw": body},
        )

    # ────────────────────────────────────────────────────────────────────

    @staticmethod
    def _validate_amount(amount: Decimal, currency: str) -> tuple[Decimal, str]:
        try:
            value = Decimal(amount)
        except Exception:  # noqa: BLE001
            return Decimal(0), "invalid_amount"
        if value <= 0:
            return Decimal(0), "invalid_amount"
        if not (currency or "").strip():
            return Decimal(0), "invalid_currency"
        return value, ""

    @staticmethod
    def _msg(code: str) -> str:
        return {
            "invalid_amount":     "المبلغ غير صحيح.",
            "invalid_currency":   "العملة غير صحيحة.",
            "missing_payment_id": "معرّف الدفع مفقود.",
            "invalid_signature":  "توقيع رد بنك فلسطين غير صحيح.",
            "http_error":         "تعذّر الاتصال ببنك فلسطين.",
            "provider_error":     "ردّ بنك فلسطين بخطأ.",
            "not_configured":     "بيانات بنك فلسطين ناقصة.",
        }.get(code, "خطأ غير معروف من بوابة بنك فلسطين.")

    def _signature_ok(self, creds: dict[str, str], raw: dict[str, Any]) -> bool:
        provided = str(raw.get("signature") or "").strip()
        if not provided:
            return False
        canonical = {k: v for k, v in raw.items() if k != "signature"}
        expected = self._sign(creds["shared_key"], canonical)
        return hmac.compare_digest(expected, provided)

    @staticmethod
    def _sign(secret: str, payload: dict[str, Any]) -> str:
        items = "&".join(f"{k}={payload[k]}" for k in sorted(payload))
        return hmac.new(secret.encode("utf-8"), items.encode("utf-8"),
                        hashlib.sha256).hexdigest()

    @staticmethod
    def _normalize_status(raw: str) -> str:
        r = (raw or "").upper()
        if r in {"APPROVED", "PAID", "CAPTURED", "SUCCESS"}:
            return "paid"
        if r in {"DECLINED", "FAILED", "CANCELLED", "EXPIRED"}:
            return "failed"
        if r in {"REFUNDED", "REVERSED"}:
            return "refunded"
        return "pending"


__all__ = ["BankOfPalestineAdapter"]
